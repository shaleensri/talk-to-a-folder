...
// MIME types this viewer can render
const RENDERABLE: Record<string, 'text' | 'pdf' | 'preview' | 'table'> = {
  'text/plain': 'text',
  'text/markdown': 'text',
  'text/csv': 'table',
  'application/pdf': 'pdf',
  'application/vnd.google-apps.document': 'preview',
  'application/vnd.google-apps.spreadsheet': 'table',
  'application/vnd.openxmlformats-officedocument.wordprocessingml.document': 'preview',
  'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': 'table',
  'application/vnd.ms-excel': 'table',
}

function viewerType(mimeType: string): 'text' | 'pdf' | 'preview' | 'table' | 'unsupported' {
...
// ---------------------------------------------------------------------------
// HTML preview viewer (Google Docs, DOCX exported to HTML)
// ---------------------------------------------------------------------------
function HtmlViewer({ html }: { html: string }) {
  return (
    <div className="h-full overflow-y-auto">
      <div
        className="p-6 max-w-3xl mx-auto prose prose-invert prose-sm prose-zinc"
  ...
  useEffect(() => {
    ...
    fetch(`/api/files/${file.id}/preview`)
      .then(async (res) => {
        ...
        else if (data.type === 'html') setPreview({ type: 'html', content: data.content })
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950 min-h-0">
      ...
      <div className={cn('flex-1 min-h-0', !file && 'flex items-center justify-center')}>
        ...
        {file && !loading && preview?.type === 'html' && (
          <HtmlViewer html={preview.content} />
        )}
